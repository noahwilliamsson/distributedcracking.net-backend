...
$q = "SELECT onlinerainbowtables.id, hash, plaintext, hash_id, hashes.job_id FROM onlinerainbowtables LEFT JOIN hashes ON hash_id=hashes.id LEFT JOIN jobs ON job_id=jobs.id WHERE found=0 AND (dt_checked IS NULL OR dt_checked < DATE_SUB(NOW(), INTERVAL 2 DAY)) AND hashtype='raw-md5' ORDER BY dt_checked";
