/*
 * Original code by suid@hack.se
 * Modifications for distributedcracking.net by noah@hack.se
 *
 */
...
*/
	...
	if((pos = sscanf(argv[4], "%d,%d,%d,%d,%d,",
				&inc_rec_state.pos,
				&inc_rec_state.numbers_cache,
				&inc_rec_state.cc_0,
				&inc_rec_state.cc_1,
				&inc_rec_state.cc_2)) != 5) {
	...
	memcpy(inc_rec_state.key_i, ptr, strlen(ptr) + 1);
